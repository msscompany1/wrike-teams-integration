...
class WrikeBot extends TeamsActivityHandler {
  async handleTeamsMessagingExtensionFetchTask(context) {
    ...
    // Get message and prefill description
  ...
  async handleTeamsMessagingExtensionSubmitAction(context, action) {
    ...
    try {
      ...
      // Ensure responsibles is always an array
      const assigneeIds = typeof assignee === "string" ? assignee.split(',').map(id => id.trim()).filter(Boolean) : assignee;
      ...
      // Get assignee names for confirmation card
      const users = await this.fetchWrikeUsers(wrikeToken);
